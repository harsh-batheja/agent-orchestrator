...
const GlobalProjectEntrySchema = z
  ...
  .passthrough(); // Preserve unknown fields on existing installs to avoid silent data loss
 ...
 *
 * Note: this is not protected by a file lock. Concurrent `ao start` invocations
 * can race on read-modify-write cycles, with the second write silently clobbering
 * the first. In practice this is rare (users typically run one project at a time),
 * but a proper advisory lock (e.g. proper-lockfile) would eliminate the race.
 */
...
export function getShadowFilePath(projectId: string): string {
  // Reject IDs with path separators, traversal patterns, or dot prefixes.
  // A leading dot would create hidden files and could shadow temp files
  // (e.g. ".foo" → ".foo.yaml" adjacent to ".foo-<hex>.yaml.tmp").
  if (!projectId || /[/\\]/.test(projectId) || projectId === ".." || projectId === "." || projectId.startsWith(".")) {
...
export function findLocalConfigUpwards(
  ...
  let depth = 0;
  while (dir !== dirname(dir) && depth < MAX_UPWARD_TRAVERSAL) {
